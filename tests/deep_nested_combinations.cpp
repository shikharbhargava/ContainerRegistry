...
// ------------------------------------------------------------
// Chain: map -> custom_struct (ResourceTracker which holds a map internally)
//
// The registry sees ResourceTracker as is_sized_container and calls
// tracker.size() — it does NOT recurse into the internal map.
// This verifies the boundary between recursive standard containers
// and opaque custom-sized types.
// ------------------------------------------------------------
TEST_F(ContainerRegistryTest, ChainMapCustomStructWithInternalMap)
{
  auto &registry = ContainerRegistry::instance();
  registry.clearAll();
  ...
  ResourceTracker rt1;
  rt1.resources["cpu"]    = 8;
  rt1.resources["memory"] = 16;   // rt1.size() = 2

  ResourceTracker rt2;
  rt2.resources["disk"] = 512;    // rt2.size() = 1

  std::map<std::string, ResourceTracker> m;
  m["server_a"] = rt1;
  m["server_b"] = rt2;

  registry.register_container("map_resource_tracker", m);

  auto all_sizes = registry.compute_all();
  // Total: rt1.size() + rt2.size() = 2 + 1 = 3
  EXPECT_EQ(all_sizes["map_resource_tracker"], 3);
  EXPECT_EQ(compute_size_recursive(m), 3);
}

// ------------------------------------------------------------
// Chain: map -> vector -> custom_struct -> (opaque internal map)
//
// Extends ChainMapVectorCustomStruct with ResourceTracker (which
// internally holds a map). The registry stops recursing at the
// ResourceTracker boundary and calls tracker.size().
// ------------------------------------------------------------
TEST_F(ContainerRegistryTest, ChainMapVectorCustomStructWithInternalMap)
{
  auto &registry = ContainerRegistry::instance();
  registry.clearAll();
  ...
  rt1.resources["a"] = 1;
  rt1.resources["b"] = 2;
  rt1.resources["c"] = 3;  // rt1.size() = 3

  ResourceTracker rt2;
  rt2.resources["x"] = 10; // rt2.size() = 1

  ResourceTracker rt3;
  rt3.resources["p"] = 7;
  rt3.resources["q"] = 8;  // rt3.size() = 2

  std::map<std::string, std::vector<ResourceTracker>> m;
  m["group_a"] = {rt1, rt2};  // 3 + 1 = 4
  m["group_b"] = {rt3};       // 2

  registry.register_container("map_vec_resource_tracker", m);

  auto all_sizes = registry.compute_all();
  // Total: 4 + 2 = 6
  EXPECT_EQ(all_sizes["map_vec_resource_tracker"], 6);
  EXPECT_EQ(compute_size_recursive(m), 6);
}
